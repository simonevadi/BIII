...
fig.suptitle('Signature6 - AI-XPRESS/MySpot with Pinhole comparison', size=16)
x_range = [2500, 40000]
...
focus_path = os.path.join('plot', 'MySpot_DetectorAtFocus-RawRaysOutgoing.csv')
focus = pd.read_csv(
    focus_path,
    sep='\t',        # columns separated by tabs
    decimal='.',     # use comma as decimal separator
...
hor_foc = np.mean(BL_df_si111['HorizontalFocusFWHM']*1e3)
ver_foc = np.mean(BL_df_si111['VerticalFocusFWHM']*1e3)
ax8.set_title(f'MySpot Focus, FWHM(HxV) {20:.0f} x {20:.0f} µm²')
