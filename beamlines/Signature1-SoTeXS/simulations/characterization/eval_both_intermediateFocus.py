...
BL_file_path = os.path.join('RAYPy_Simulation_sotexs_2400', 'IntermediateFocus_RawRaysIncoming.csv')
BL_df = pd.read_csv(BL_file_path)

BL_file_path = os.path.join('RAYPy_Simulation_sotexs_1200', 'IntermediateFocus_RawRaysIncoming.csv')
...
fig.suptitle('Signature2 - Liquid Interface, 2400 l/mm . Intermediate Focus', size=16)
x_range = [0, 10000]
...
# Focus
ax7 = axs[3, 0]
focus_path = os.path.join('plot', 'IntermediateFocus-RawRaysIncoming.csv')
focus = pd.read_csv(
    focus_path,
...
hb = ax7.hexbin(
    focus['IntermediateFocus_OX']*1e3,
    focus['IntermediateFocus_OY']*1e3,
    gridsize=60, cmap='viridis'
)
ax7.set_xlabel('µm')
ax7.set_ylabel('µm')
hor_foc = np.mean(BL_df['HorizontalFocusFWHM']*1e3)
ver_foc = np.mean(BL_df['VerticalFocusFWHM']*1e3)
...
# Save the the figure
plt.tight_layout()
plt.savefig('plot/sotexs_intermediateFocus.png')
# plt.show()
plt.close()
