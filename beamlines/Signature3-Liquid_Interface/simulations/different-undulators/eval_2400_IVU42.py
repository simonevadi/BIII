...
from parameter import undulator_IVU42_HL as undulator_df
...
BL_file_path = os.path.join('RAYPy_Simulation_elisa_2400_IVU42', 'DetectorAtFocus_RawRaysOutgoing.csv')
...
ax2.set_title(f"IVU42 Flux curve- min E {undulator_df[f'Energy1[eV]'].iloc[-1]}")
...
# Save the the figure
plt.tight_layout()
plt.savefig('plot/elisa_2400_IVU42.png')
# plt.show()
plt.close()
...
ax.plot(efficiency_2400['Energy[eV]'],
...
ax.plot(efficiency_2400['Energy[eV]'],
         efficiency_2400['alpha_deg'])

ax.set_title('Alpha grazing [deg]')
ax.set_xlabel('Energy [eV]')
ax.set_ylabel('[%]')
ax.set_xlim(x_range)
ax.minorticks_on()
...
ax.plot(efficiency_2400['Energy[eV]'],
         efficiency_2400['beta_deg'])

ax.set_title('Beta grazing [deg]')
ax.set_xlabel('Energy [eV]')
ax.set_ylabel('[%]')
ax.set_xlim(x_range)
ax.minorticks_on()
