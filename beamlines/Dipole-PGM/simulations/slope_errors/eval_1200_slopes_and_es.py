...
all_sim = all_sim[all_sim['Dipole.photonEnergy'] < 2200]
...
for es in exit_slit:
    ...
    plot_folder = f'plot/slopes/BIII-Dipole-PGM-1200-slopes-es-{int(es*1000)}µm'
    os.makedirs(plot_folder, exist_ok=True)
    fig, (axs) = plt.subplots(4, 1,figsize=(12,12))

    # No Slopes Errors
    filtered_sim = filter_df(sim, cols=list(el_dict.keys()))
    extract_and_plot(filtered_sim, 'Dipole', axs, 'No Slopes Errors')
    ...
    extract_and_plot(filtered_sim, 'Dipole', axs, f'All Slopes Errors')

    decorate_and_save_plot(axs, title=f'BIII-Dipole-PGM, ES={np.round(es*1000, 0)} µm', 
                        savepath=os.path.join(plot_folder, f'BIII-Dipole-PGM-1200-slopes-ES_{np.round(es*1000, 0)} µm.png'), 
                        showplot=False)
    ...
    for element,slopes in el_dict.items():
        ...
        for slope in slopes:
            filtered_sim = filter_df(sim, 
                                    ...
                                    col_to_set=element, 
                                    value=slope)
            extract_and_plot(filtered_sim, 'Dipole', axs, f'{element} {np.round(slope,2)} rms')

        title = f"SoTeXS, {element.split('.')[0]} {element.split('.')[1]} slope Error, ES={int(es*1000)}µm"
        decorate_and_save_plot(axs, title=title, 
                            savepath=os.path.join(plot_folder, f'BIII-Dipole-PGM-1200-slopes-{element}-ES_{np.round(es*1000, 0)} µm.png'), 
                            showplot=False)
